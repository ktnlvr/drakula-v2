export default `
    ...
    float createWave(float yPos, float offset) {
        ...
        float wave = smoothstep(0.0, 0.3, 1.0 - abs(yPos - riseFactor) / 2.0);
        float topFade = smoothstep(1.75, 0.5, yPos); // Adjust these values to control fade height
...
void main() {
    ...
    vec3 baseColor = vec3(0.256, 0.38, 0.311);  
    vec3 topColor = vec3(0.256, 0.38, 0.311);
    vec3 color = mix(baseColor, topColor, vPosition.y * 0.1 + 0.5);

    vec3 waveColor = vec3(0.256, 0.38, 0.311);  
    float waveStrength = (wave1 * 0.95 + wave2 * 0.95);
